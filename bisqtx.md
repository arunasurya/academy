...
A Bitcoin transaction consists of inputs and outputs. An input of a transaction is actually the output of the previous transaction. An unspent transaction output is called a utxo. A utxo can be spent by using it as an input to a new transaction.   The sum of all your utxos is the sum of you all the bitcoin you own. You need to spend the entire utxo

If you want to send bitcoin to someone, you need to create a new transaction. You can use one of your utxos as the sole input to the new transaction or you can use several utxos as several inputs. When you create a transaction, you specify a lock script that, if unlocked, releases the bitcoin.
